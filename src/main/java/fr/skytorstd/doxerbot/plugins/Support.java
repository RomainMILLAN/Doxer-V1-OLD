...
public class Support extends ListenerAdapter {
        ...
        if(e.getName().equals("support")){
            if(ConfigurationPluginsDatabase.getStatePluginWithPluginName(PluginName.SUPPORT.getMessage(), e.getGuild().getId())){
                ...
                if(action.equals("ticket-message")){
                    ...
                    Channel channelToRemove = e.getChannel();

                    if(channelToRemove.getName().startsWith("\uD83D\uDCE8-ticket")){
                        deleteCurrentSupport(channelToRemove);

                        e.getMember().getUser().openPrivateChannel().queue((channel) -> {
                            channel.sendMessageEmbeds(SupportCrafter.craftDeleteTicket(e.getGuild())).queue();
                        });
                        Logger.getInstance().toLog(PluginName.SUPPORT.getMessage(), SupportMessages.TICKET_DELETE_SUCCESS.getMessage(), e.getGuild(), e.getMember(), true);
                    }else {
                        e.replyEmbeds(EmbedCrafter.embedCraftWithDescriptionAndColor(":x: " + SupportMessages.TICKET_DELETE_ERROR.getMessage(), Color.RED)).queue((m) -> m.deleteOriginal().queueAfter(QueueAfterTimes.SUCCESS_TIME.getQueueAfterTime(), TimeUnit.SECONDS));
                        Logger.getInstance().toLog(PluginName.SUPPORT.getMessage(), SupportMessages.TICKET_DELETE_ERROR.getMessage(), e.getGuild(), e.getMember(), false);
                    }
                }else{
    ...
    /**
     * Permet de crée un channel ticket
     * <pre/>
     *
     * @param guild
     * @param category
    ...
    /**
     * Permet de supprimer un channel ticker
     * <pre/>
     * @param channel
     */
    private static void deleteCurrentSupport(Channel channel){
